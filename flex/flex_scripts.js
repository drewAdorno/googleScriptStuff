function sendProgressUpdates() {
  ...
  if (result === SpreadsheetApp.getUi().Button.OK) {
      for (let student of emailStudents) {
          ...
          MailApp.sendEmail(email, `${stack} - Weekly Progress Report`, message, {cc: `${studentSupportEmail}, ${instructorEmail}`, htmlBody: message});
      }
      SpreadsheetApp.getActive().toast(`${emailStudents.length} emails were sent.`);
  } 
  else {
      ...
      if (result === SpreadsheetApp.getUi().Button.OK){
        ...
        for (let student of emailStudents){
          ...
          if(status == 'PROBATION (AP)'){
            ...
            subjectLine,
            message,
            {htmlBody: message, cc:emailCC ,attachments: [file.getAs(MimeType.PDF)]}
            );
          }
          else{
            MailApp.sendEmail(email, subjectLine, message, {htmlBody: message, cc: emailCC}
            );
          } 
        }
